// K-Ripper — analysis worker (tempo + key for any audio format ffmpeg can decode: m4a/opus/mp3/wav), run OFF the main thread.
//
...
async function main() {
  ...
  let samples = null;
  try {
    const r = spawnSync(ffmpegPath, ffmpegAnalysisArgs(audioPath), { maxBuffer: 1 << 28, windowsHide: true });
    const b = r.stdout;
